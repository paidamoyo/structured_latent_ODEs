...
class MechanisticModel(nn.Module):
    ...
    def guide(self, observations, symptoms, shedding):
        ...
        with pyro.plate("data"):
            ...
            z_shedding, z_symptoms, z_epsilon, z_u = z['z_shedding'], z['z_symptoms'], z['z_epsilon'], z['z_u']
    ...
    def split_z_prior(self, z):
        ...
        return {"z_shedding": z_shedding,
                "z_symptoms": z_symptoms,
                }

    def split_z(self, loc_z, scale_z):
        # z_shedding, z_symptoms, z_epsilon, z_u
        loc_z_shedding, scale_z_shedding = loc_z[:, 0:self.z_shedding_dim], scale_z[:, 0:self.z_shedding_dim]

        z_group_dim = self.z_shedding_dim + self.z_symptoms_dim
        loc_z_symptoms, scale_z_symptoms = loc_z[:, self.z_shedding_dim: z_group_dim], scale_z[:,
                                                                                       self.z_shedding_dim: z_group_dim]
        ...
        return {"z_shedding": [loc_z_shedding, scale_z_shedding],
    ...
    def model_meta(self, observations, shedding, symptoms):
        ...
        with pyro.plate("data"):
            ...
            z_shedding = pyro.sample("z_shedding_u", dist.Normal(z_shedding[0], z_shedding[1]).to_event(1))
            z_symptoms = pyro.sample("z_symptoms_u", dist.Normal(z_symptoms[0], z_symptoms[1]).to_event(1))

            self.q_label(shedding=shedding, symptoms=symptoms, z_symptoms=z_symptoms, z_shedding=z_shedding, name='q')

    def q_label(self, shedding, symptoms, z_shedding, z_symptoms, name):
        alpha_shedding = self.q_shedding_given_z_shedding.forward(z_shedding)  # q(shedding|z_shedding)
        alpha_symptoms = self.q_symptom_given_z_symptom.forward(z_symptoms)

        with pyro.poutine.scale(scale=self.aux_loss_multiplier):
            pyro.sample("shedding_u_" + name, dist.Bernoulli(alpha_shedding).to_event(1), obs=shedding)

        with pyro.poutine.scale(scale=self.aux_loss_multiplier):
            pyro.sample("symptoms _u_" + name, dist.Bernoulli(alpha_symptoms).to_event(1), obs=symptoms)
