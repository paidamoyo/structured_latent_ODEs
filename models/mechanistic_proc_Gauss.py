...
class MechanisticModelGauss(nn.Module):
    ...
    def setup_networks(self):
        ...
        self.q_C12_given_z_C12 = EncoderMLP(
            mlp_sizes=[self.z_C12_dim]
            + [self.u_hidden_dim]
            + [[self.C12_dim, self.C12_dim]],
            activation=nn.Softplus,
        ...
        self.q_C6_given_z_C6 = EncoderMLP(
            mlp_sizes=[self.z_C6_dim]
            + [self.u_hidden_dim]
            + [[self.C6_dim, self.C6_dim]],
            activation=nn.Softplus,
            output_activation=[Exp, Exp],
            allow_broadcast=self.allow_broadcast,
            use_cuda=self.use_cuda,
        ...
        self.constant_std_C_12 = torch.nn.Parameter(
            torch.ones(1) * self.config.constant_std, requires_grad=True
        )
        self.constant_std_C_6 = torch.nn.Parameter(
            torch.ones(1) * self.config.constant_std, requires_grad=True
        )
    ...
    def model(self, observations, aR, aS, C12, C6):
        ...
        with pyro.plate("data"):
            ...
            self.q_label(
                aR=aR, aS=aS, z_aR=z_split["z_aR"], z_aS=z_split["z_aS"], name="p"
            )
            self.q_continous(
                C12=C12, C6=C6, z_C12=z_split["z_C12"], z_C6=z_split["z_C6"], name="p"
            )
            # import ipdb;
            # ipdb.set_trace()
    ...
    def guide(self, observations, aR, aS, C12, C6):
        ...
        with pyro.plate("data"):
            ...
            z_aR, z_aS, z_C12, z_C6, z_epsilon, z_u = (
                z["z_aR"],
                z["z_aS"],
                z["z_C12"],
                z["z_C6"],
    ...
    def split_z_prior(self, z):
        ...
        return {"z_aR": z_aR, "z_aS": z_aS, "z_C12": z_C12, "z_C6": z_C6}

    def split_z(self, loc_z, scale_z):
        loc_z_aR, scale_z_aR = (
            loc_z[:, 0 : self.z_aR_dim],
            scale_z[:, 0 : self.z_aR_dim],
        )

        z_group_dim = self.z_aR_dim + self.z_aS_dim
        loc_z_aS, scale_z_aS = (
            loc_z[:, self.z_aR_dim : z_group_dim],
            scale_z[:, self.z_aR_dim : z_group_dim],
        )

        z_group_C12_dim = z_group_dim + self.z_C12_dim
        loc_z_C12, scale_z_C12 = (
            loc_z[:, z_group_dim:z_group_C12_dim],
            scale_z[:, z_group_dim:z_group_C12_dim],
        )

        z_group_cond_dim = z_group_C12_dim + self.z_C6_dim
        loc_z_C6, scale_z_6 = (
            loc_z[:, z_group_C12_dim:z_group_cond_dim],
            scale_z[:, z_group_C12_dim:z_group_cond_dim],
        )
    ...
    def model_meta(self, observations, aR, aS, C12, C6):
        ...
        with pyro.plate("data"):
            ...
            z = self.split_z(loc_z=loc_z, scale_z=scale_z)
            z_aR, z_aS, z_C12, z_C6, z_epsilon = (
                z["z_aR"],
                z["z_aS"],
                z["z_C12"],
                z["z_C6"],
    ...
    def q_label(self, aR, aS, z_aR, z_aS, name):
        ...
        with pyro.poutine.scale(scale=self.aux_loss_multiplier):
            pyro.sample(
                "aR_u_" + name, dist.OneHotCategorical(alpha_aR).to_event(1), obs=aR
            )

        with pyro.poutine.scale(scale=self.aux_loss_multiplier):
            pyro.sample(
                "aS_u_" + name, dist.OneHotCategorical(alpha_aS).to_event(1), obs=aS
            )
    ...
    def q_continous(self, C12, C6, z_C12, z_C6, name):
        ...
        with pyro.poutine.scale(scale=self.aux_loss_multiplier):
            pyro.sample(
                "C12_u_" + name, dist.Laplace(loc_C12, std_C12).to_event(1), obs=C12
            )
            pyro.sample(
                "C6_u_" + name, dist.Laplace(loc_C6, std_C6).to_event(1), obs=C6
            )
